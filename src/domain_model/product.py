...
class ThreeWayRecognitionStrategy(RecognitionStrategy):
    ...
    def calculate_revenue_recognitions(self, contract: Contract):
        ...
        contract.add_revenue_recognition(
            RevenueRecognition(allocations[0], contract.get_when_signed())
        )
        contract.add_revenue_recognition(
            RevenueRecognition(allocations[1], contract.get_when_signed().add_days(self._first_recognition_offset)))
        contract.add_revenue_recognition(
            RevenueRecognition(allocations[2], contract.get_when_signed().add_days(self._second_recognition_offset)))
